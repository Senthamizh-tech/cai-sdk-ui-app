                    ...
                    if (tempData.from === "bot" && tempData.type === "bot_response") {
                        ...
                        if (tempData && tempData.message && tempData.message.length) {
                            ...
                            if (tempData.message[0]) {
                                ...
                                // ##############################
                                // ###Calling veribiage component
                                // ##############################
                                if ((tempData.message[0].component.payload.text).startsWith("ESI_")) {
                                    let verbiageBuilderData = [];
                                    verbiageBuilderData = me.verbiageBuilder();
                                    let result = verbiageBuilderData.filter((ele) => ele.RESPONSE_ID === tempData.message[0].component.payload.text)
                                    result.length > 0 ? tempData.message = me.transformerBuilder(result) : tempData.message[0].cInfo.body = tempData.message[0].component.payload.text;
                                }
                            }
            ...
            chatWindow.prototype.verbiageBuilder = function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        verbiageBuilderResp = data.data;
                    },
                    error: function (err) {
                        console.log("err", err)
                    }
                });
                return verbiageBuilderResp;
            }

            chatWindow.prototype.transformerBuilder = function (templateData) {
                //hook to add custom events
                var me = this;
                var _chatContainer = me.config.chatContainer;
                var templateType = templateData[0].MEDIA_TYPE

                var tableTemplate = templateData[0].DATA ? [{
                    "type": "text",
                    "component": {
                        "type": "template",
                        "payload": {
                ...
                var dafaultTextTemplate = [
                    {
                        ...
                        "cInfo": {
                            "body": templateData[0].WEB_RESPONSE_MSG
                        }
                    }
                ];
